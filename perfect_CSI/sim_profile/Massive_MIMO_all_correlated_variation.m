...
function Massive_MIMO_all_correlated_variation(par, runId)
...
rho = [0:0.05:par.rho_max];
% initialize result arrays (detector x SNR)
res.BER = zeros(length(par.detector),length(rho)); % bit error rate


Rtx = zeros(par.MT, par.MT,length(rho));
Rrx = zeros(par.MR, par.MR, length(rho));
for k = 1:length(rho)
    for p = 1:par.MT
        for q  = 1:par.MT
            if p==q
...
for d=1:length(par.detector)
    if d==1
        semilogy(rho,Results.BER(d,:),marker_style{d},'LineWidth',2)
        hold on
    else
...
hold off
grid on
xlabel('Average correlation','FontSize',12)
ylabel('bit error rate (BER)','FontSize',12)
axis([min(rho) max(rho) 1e-6 1])
