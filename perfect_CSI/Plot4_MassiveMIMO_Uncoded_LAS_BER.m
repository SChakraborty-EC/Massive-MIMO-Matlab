...
par.SNRdB_list = 6:2:25;
par.band_approx =0;   % 1 means band matrix approximation, 0 --> no approximation
par.simName = ['Plot4_BER_LAS_full_',num2str(par.MT),'x', num2str(par.MR), '_',num2str(par.mod), '_RunID_'];
disp(par.simName)
Massive_MIMO_all(par,0)

par.band_approx =1;   % 1 means band matrix approximation, 0 --> no approximation
par.w=16;
par.simName = ['Plot4_BER_LAS_partial_w_', num2str(par.w),num2str(par.MT),'x', num2str(par.MR), '_',num2str(par.mod), '_RunID_'];
disp(par.simName)
Massive_MIMO_all(par,0)
...
par.band_approx =1;   % 1 means band matrix approximation, 0 --> no approximation
par.w=8;
par.simName = ['Plot4_BER_LAS_partial_w_', num2str(par.w),'_',num2str(par.MT),'x', num2str(par.MR), '_',num2str(par.mod), '_RunID_'];
disp(par.simName)
Massive_MIMO_all(par,0)
...
% 
%  clear
%  fullname = ['results/Plot4_BER_LAS_full_32x128_64QAM_RunID_0.mat'];
%  
%  tmp = load(fullname);
%  tmp.Results.BER
%  close all
...
% set(gca,'FontSize',12)
% 
%  fullname = ['results/Plot4_BER_LAS_partial_w_1632x128_64QAM_RunID_0.mat'];
%  
%  tmp = load(fullname);
%  tmp.Results.BER
% 
