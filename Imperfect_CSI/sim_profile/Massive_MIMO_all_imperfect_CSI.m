...
function Massive_MIMO_all_imperfect_CSI(par, runId)
...
bits = randi([0 1],par.MT,par.Q,par.block_size,par.trials);

% trials loop
tic
for t=1:par.trials

    s = zeros(par.MT,par.block_size);
    for i = 1:par.block_size
        % generate transmit symbol
        idx = bi2de(bits(:,:,i,t),'left-msb')+1;
        s(:,i) = par.symbols(idx).';
    end
    transmit_data_length = par.block_size+par.PilotNumber;
    % generate iid Gaussian channel matrix & noise vector
    n = sqrt(0.5)*(randn(par.MR,transmit_data_length)+1i*randn(par.MR,transmit_data_length));
    Hiid = sqrt(0.5)*(randn(par.MR,par.MT)+1i*randn(par.MR,par.MT));
    H = par.Rrx^(1/2) * Hiid * par.Rtx^(1/2);
    % transmit over noiseless channel (will be used later)
    par.pilot = sqrt(par.MT*par.Es)*par.P;
    x = H*[par.pilot s];
    ...
    for k=1:length(par.SNRdB_list)
        ...
        S = yBlock(:, 1:par.PilotNumber);
        H = (par.MT/par.Pnorm2)*S*par.pilot'/(par.MT*par.Es);
        
        yBlock_data = yBlock(:,par.PilotNumber+1:par.PilotNumber+par.block_size);
        ...
        for d=1:length(par.detector)
            ...
            switch (par.detector{d}) % select algorithms
                case 'MMSE'
                    bithat = MMSE_imperfect_CSI(par,H,yBlock_data,N0);

                case 'SIMO' % SIMO lower bound
                    bithat = SIMO(par,H,y,N0,s);
                    
                    
                case 'Gauss-Seidel1' % Gauss-Seidel detector
                    bithat = Gauss_Seidel_imperfect_CSI(par,H,yBlock_data,N0);

                case 'ADMIN' % ADMM-based Infinity Norm detector
                    bithat = ADMIN(par,H,yBlock_data,N0);

                case 'OCDBOX' % co-ordinate descent (optimized) detector
                    bithat = OCDBOX(par,H,yBlock_data);

                case 'Neumann' % coordinate descent
                    bithat = Neumann(par,H,yBlock_data,N0);

                case 'Conjugate-Gradient' % conjugate gradient detector
                    bithat = CG_imperfect_CSI(par,H,yBlock_data,N0);

                case 'richardson' % conjugate gradient detector
                    bithat = richardson_imperfect_CSI(par,H,yBlock_data,N0);

                case 'SOR' % conjugate gradient detector
                    bithat = SOR_imperfect_CSI(par,H,yBlock_data,N0);

                case 'Newton_iteration' % conjugate gradient detector
                    bithat = Newton_iteration_imperfect_CSI(par,H,yBlock_data,N0);

                case 'jacobi' % conjugate gradient detector
                    bithat = jacobi_imperfect_CSI(par,H,yBlock_data,N0);

                case 'chebyshev' % conjugate gradient detector
                    bithat = chebyshev_imperfect_CSI(par,H,yBlock_data,N0);

                case 'Newton_richardson' % conjugate gradient detector
                    bithat = Newton_richardson_imperfect_CSI(par,H,yBlock_data,N0);

                 case 'Newton_GS' % conjugate gradient detector
                    bithat = Newton_GS_imperfect_CSI(par,H,yBlock_data,N0);
                   
                case 'GS_richardson' % conjugate gradient detector
                    bithat = GS_richardson_imperfect_CSI(par,H,yBlock_data,N0);

                case 'Newton_chebyshev' % conjugate gradient detector
                    bithat = Newton_chebyshev_imperfect_CSI(par,H,yBlock_data,N0);

                case 'MMSEapprox', % biased MMSE detector
                    bithat = MMSE_approx_imperfect_CSI(par,H,yBlock_data, N0);

                case 'steepest_jacobi' % conjugate gradient detector
                    bithat = steepest_jacobi_imperfect_CSI(par,H,yBlock_data,N0);

                    
                    
                case 'LAS-CG', % ML detection using sphere decoding
                    par.MMSE_init = 'Conjugate-Gradient';
                    bithat = LAS_imperfect_CSI(par,H,yBlock_data, N0);
        ...
        res.BER(d,k) = res.BER(d,k) + sum(sum(sum(bits(:,:,:,t)~=bithat)))/(par.MT*par.Q*par.block_size);
